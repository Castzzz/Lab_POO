...
public class Biblioteca {
    ...
    private Map<Pessoa, List<Livro>> livrosEmprestadosByUsuario;
    ...
    public Biblioteca(){
        ...
        this.livrosEmprestadosByUsuario = new HashMap<>();
    }
    ...
    public void adquirirLivro(Livro livro, int quantidade) {
        ...
        int quantidadeAtual = this.quantidadeByLivro.getOrDefault(livro, 0);
        this.quantidadeByLivro.put(livro, quantidadeAtual + quantidade);
    ...
    public boolean emprestarLivro(Livro livro, Pessoa usuario)
        ...
        if (!this.usuarioByCpf.containsKey(usuario.getCpf())) {
            throw new UsuarioNaoCadastradoException();
        }

        if (getQuantidadeDeLivrosDevidos(usuario) >= MAX_LIVROS_DEVIDOS_POR_USUARIO) {
            throw new LimiteEmprestimosExcedidoException();
        }

        int quantidadeNaEstante = getQuantidadeDeLivrosNasEstantes(livro);

        if (quantidadeNaEstante < MIN_COPIAS_PARA_PODER_EMPRESTAR) {
            return false;
        }

        this.quantidadeByLivro.put(livro, quantidadeNaEstante - 1);
        this.totalLivrosNasEstantes--;

        List<Livro> livrosDoUsuario = this.livrosEmprestadosByUsuario.getOrDefault(usuario, new ArrayList<>());
        livrosDoUsuario.add(livro);
        this.livrosEmprestadosByUsuario.put(usuario, livrosDoUsuario);
    ...
    public void receberDevolucaoLivro(Livro livro, Pessoa usuario) throws DevolucaoInvalidaException {
        ...
        if (!livrosDoUsuario.remove(livro)) {
    ...
    public int getQuantidadeDeLivrosDevidos(Pessoa usuario) {
        List<Livro> livrosDoUsuario = this.livrosEmprestadosByUsuario.get(usuario);
        return (livrosDoUsuario != null) ? livrosDoUsuario.size() : 0;
    }
    ...
    public int getQuantidadeDeLivrosNasEstantes(Livro livro) {
        return this.quantidadeByLivro.getOrDefault(livro, 0);
    }
}